 ...
 * onMeasuer（int，int）测量：该方法来检查view组件及他所包含的所有子组建的大小
 * onLayout(boolean,int,int,int,int)位置：当该组件要分配器子组件的位置，大小时，调用（使用频率较少）
 * onDraw(canves)绘制:当该组件将要绘制他的内容时，回掉该方法（使用频率最高，不要再此方法里做耗时操作和对象的建立）
 * <p>
 * 创建一个可以进行拖动的小球
 * <p>
 * 自绘式自定义控件实现步骤：
...
/*                event.getRawX();
                ...
                Toast.makeText(getContext(), "点到我了"+ mOnBall, Toast.LENGTH_SHORT).show();
                break;
